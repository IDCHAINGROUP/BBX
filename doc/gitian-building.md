...
*Setup instructions for a gitian build of ballotbox using a VM or physical system.*

Gitian is the deterministic build process that is used to build the ballotbox
...
to the ballotbox GitHub release page.
...
- [Building ballotbox](#building-ballotbox-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/ballotbox-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/ballotbox-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/ballotbox-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/ballotbox-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/ballotbox-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/ballotbox-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/ballotboxvps-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@ballotbox.dev) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/ballotbox-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/ballotbox-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/ballotbox-osx-*-build.assert
```
