...
The build script we'll be using is contained in the ballotbox github repository ([contrib/gitian-build.py](https://github.com/ballotboxcoin/bbx/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the ballotbox repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/ballotboxcoin/bbx/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, ballotbox-detached-sigs, and ballotbox GitHub repos.
