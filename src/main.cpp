...
#if defined(NDEBUG)
#error "ballotbox cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uballotbox) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateBBXSupply(int nHeightStart)
{
	AssertLockHeld(cs_main);
	...
	uiInterface.ShowProgress(_("Recalculating BBX supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating BBX supply..."), percent);
		}
...
CAmount GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
        ...
        nSubsidy = 2000.5 * COIN;
    } else if (nHeight <= 50000) {
        nSubsidy = 1500.5 * COIN;
...
CAmount GetDevPayment(int nHeight, CAmount blockValue)
{
    ...
    if (nHeight <= 1000) { 
        ...
        nDevPayment = 500.000000001 * COIN; // symbolic < 1 satoshi to prevent rounding issues
    } else if (nHeight <= 40000) {
        nDevPayment = 1000.000000001 * COIN;
    } else if (nHeight <= 50000) {
        nDevPayment = 750.000000001 * COIN;
    } else if (nHeight <= 60000) {
        nDevPayment = 500.000000001 * COIN;
    } else if (nHeight <= 70000) {
        nDevPayment = 350.000000001 * COIN;
    } else if (nHeight <= 80000) {
        nDevPayment = 300.000000001 * COIN;
    } else if (nHeight <= 90000) {
        nDevPayment = 250.000000001 * COIN;
    } else if (nHeight <= 100000) {
        nDevPayment = 200.000000001 * COIN;
    } else if (nHeight <= 110000) {
        nDevPayment = 100.000000001 * COIN;
    } else {
        nDevPayment = 250.000000001 * COIN;
    }
...
{
    util::ThreadRename("ballotbox-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight > last_pow_block) {
      ...
      if (nDevReward > 0) {
        ...
        if (!DevPaid) {
            if (sporkManager.IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT) && pindex->nHeight > last_pow_block * 3)
                return state.DoS(100, error("ConnectBlock(): no dev reward"), REJECT_INVALID, "no-dev-reward");
            else
                LogPrint(BCLog::BENCH, "Warning: ConnectBlock(): no dev reward");
        }
      }
    }
    // Ballotbox>
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> ballotboxInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                ballotboxInputs.push_back(stakeIn);
        }
        const bool hasBBXInputs = !ballotboxInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasBBXInputs) {
                    ...
                    for (const CTxIn& ballotboxIn : ballotboxInputs)
                        if(ballotboxIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasBBXInputs) {
                            for (const CTxIn& stakeIn : ballotboxInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // ballotbox: We use certain sporks during IBD, so check to see if they are
