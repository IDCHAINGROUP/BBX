    ...
    if (hasPayment) {
        ...
        if (fProofOfStake) {
            ...
            if (nDevReward > 0) {
                ...
                txNew.vout[0].nValue = blockValue - nDevReward;
            
                LogPrint(BCLog::MASTERNODE,"Dev payment of %s to %s\n", FormatMoney(nDevReward).c_str(), EncodeDestination(destination).c_str());
            }
        }
    }
}
