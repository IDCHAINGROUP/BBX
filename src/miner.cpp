...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != g_best_block)
            return error("ballotbox CoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("ballotbox CoinMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("ballotbox CoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    util::ThreadRename("ballotbox-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running ballotbox CoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ballotbox CoinMiner exception");
    } catch (...) {
        LogPrintf("ballotbox CoinMiner exception");
    }

    LogPrintf("ballotbox CoinMiner exiting\n");
}
