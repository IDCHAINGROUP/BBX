...
#include "qt/ballotbox/splash.h"
#include "qt/ballotbox/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("ballotbox-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in ballotbox.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in ballotbox.qrc)
...
/** Class encapsulating ballotbox startup and shutdown.
...
/** Main ballotbox application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (ballotboxGUI)
...
{
    window = new ballotboxGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &ballotboxGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &ballotboxGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(ballotboxGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(ballotboxGUI::DEFAULT_WALLET);
        ...
        // ballotbox: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ballotboxGUI::handlePaymentRequest);
        connect(window, &ballotboxGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. Ballotbox Coin can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(ballotbox_locale);
    Q_INIT_RESOURCE(ballotbox);
    ...
    /// 6. Determine availability of data directory and parse ballotbox.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("Ballotbox Coin"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("Ballotbox Coin"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // ballotbox: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("Ballotbox Coin didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
