...
#include "qt/ballotbox/forms/ui_addresseswidget.h"
#include "qt/ballotbox/addresslabelrow.h"
#include "qt/ballotbox/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(ballotboxGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        CTxDestination ballotboxAdd = DecodeDestination(address.toUtf8().constData(), isStakingAddress);
        ...
        if (walletModel->updateAddressBookLabels(ballotboxAdd, label.toUtf8().constData(),
