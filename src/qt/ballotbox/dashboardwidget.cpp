...
#include "qt/ballotbox/dashboardwidget.h"
#include "qt/ballotbox/forms/ui_dashboardwidget.h"
#include "qt/ballotbox/sendconfirmdialog.h"
#include "qt/ballotbox/txrow.h"
...
DashboardWidget::DashboardWidget(ballotboxGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareBBX, "square-chart-ballotbox");
    setCssProperty(ui->labelBBX, "text-chart-ballotbox");
    ...
    setCssProperty(ui->labelAmountBBX, "text-stake-ballotbox-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &ballotboxGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &ballotboxGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &ballotboxGUI::openFAQ);
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            ballotbox = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalBBX += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesBBX.append(ballotbox);

        int max = std::max(ballotbox, ballotbox);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(30,59,145));
    ...
    set0->append(chartData->valuesBBX);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalBBX > 0) {
        setCssProperty(ui->labelAmountBBX, "text-stake-ballotbox");
    } else {
        setCssProperty(ui->labelAmountBBX, "text-stake-ballotbox-disable");
    }
    forceUpdateStyle({ui->labelAmountBBX});
    ui->labelAmountBBX->setText(GUIUtil::formatBalance(chartData->totalBBX, nDisplayUnit));
