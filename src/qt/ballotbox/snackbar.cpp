...
#include "qt/ballotbox/forms/ui_snackbar.h"
#include "qt/ballotbox/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(ballotboxGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &ballotboxGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
