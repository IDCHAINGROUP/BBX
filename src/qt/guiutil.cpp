...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter Ballotbox Coin address (e.g. %1)").arg("M6tGR83SQbie1SW72hjcWJvFfip5krte2Z"));
...
{
    // return if URI is not valid or is no ballotbox: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::BBX, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert ballotbox:// to ballotbox:
    //
    //    Cannot handle this later, because ballotbox:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BBX, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ballotbox.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for ballotbox.lnk
...
{
    return GetAutostartDir() / "ballotbox.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a ballotbox.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Ballotbox Coin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the ballotbox app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add ballotbox app to startup item list
