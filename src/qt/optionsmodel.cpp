...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::BBX);
    ...
    if (settings.contains("nAnonymizeballotboxAmount") || reset)
        SoftSetArg("-anonymizeballotboxamount", settings.value("nAnonymizeballotboxAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in ballotbox.cpp during shotdown to also remove the window positions
