...
UniValue getrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw std::runtime_error(
            ...
            "     {\n"
            "       \"value\" : x.xxx,            (numeric) The value in BBX\n"
            "       \"n\" : n,                    (numeric) index\n"
            ...
            "           \"ballotboxaddress\"        (string) ballotbox address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw std::runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of ballotbox addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) ballotbox address\n"
            "      ,...\n"
            "    ]\n"
            ...
            "    \"address\" : \"address\",  (string) the ballotbox address\n"
            "    \"account\" : \"account\",  (string) DEPRECATED. The associated account, or \"\" for the default account\n"
            "    \"scriptPubKey\" : \"key\", (string) the script key\n"
            "    \"redeemScript\" : \"key\", (string) the redeemscript key\n"
            "    \"amount\" : x.xxx,         (numeric) the transaction amount in BBX\n"
    ...
    if (params.size() > 2) {
        ...
        for (unsigned int inx = 0; inx < inputs.size(); inx++) {
            ...
            if (!IsValidDestination(dest))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Ballotbox Coin address: ") + input.get_str());
...
UniValue createrawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw std::runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric, required) The key is the ballotbox address, the value is the ballotbox amount\n"
            "      ,...\n"
            "    }\n"
    ...
    for (const std::string& name_ : addrList) {
        ...
        if (!IsValidDestination(address))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Ballotbox Coin address: ")+name_);
...
UniValue decoderawtransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) ballotbox address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
