...
bool CBBXStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CBBXStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CBBXStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CBBXStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CBBXStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CBBXStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CBBXStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CBBXStake::GetUniqueness() const
{
    //The unique identifier for a BBX stake is the outpoint
...
CBlockIndex* CBBXStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CBBXStake::ContextCheck(int nHeight, uint32_t nTime)
{
